...
def most_successful(df, sport):
    ...
    x = \
        temp_df['Athlete'].value_counts().reset_index().head(15).merge(df, left_on='index', right_on='Athlete',
                                                                       how='left')[
            ['index', 'Athlete_x', 'Sport', 'region']].drop_duplicates('index')
    x.rename(columns={'index': 'Name', 'Athlete_x': 'Medals'}, inplace=True)
    return x
...
def country_event_heat(df, country):
    temp_df = df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
    new_df = temp_df[temp_df['region'] == country]
...
def top_successful(df, country):
    ...
    x = \
        temp_df['Athlete'].value_counts().reset_index().head(10).merge(df, left_on='index', right_on='Athlete',
                                                                       how='left')[
